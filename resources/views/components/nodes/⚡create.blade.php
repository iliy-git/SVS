...
<div class="row justify-content-center animate__animated animate__fadeIn">
    <div class="col-md-9 col-lg-8">
        <div class="card border-0 shadow-lg rounded-4 overflow-hidden" style="background: #1a1d21;">
            <div class="card-header bg-primary bg-opacity-10 border-0 p-4"><h4 class="fw-bold text-white m-0">Новая
                    нода</h4></div>
                ...
                <form wire:submit.prevent="save">
                        ...
                        <div class="d-flex flex-wrap gap-2 overflow-auto py-2 px-1 custom-scrollbar"
                            ...
                            @foreach($flags as $flag)
                                <label class="m-0 cursor-pointer position-relative">
                                    <input type="radio" wire:model.live="flag_id" value="{{ $flag->id }}"
                                           class="d-none">
                                    <div
                                        class="flag-box {{ $flag_id == $flag->id ? 'active' : '' }} d-flex flex-column justify-content-center align-items-center">
                                        @if($flag_id == $flag->id)
                                            <div class="position-absolute top-0 end-0 p-1"><i
                                        ...
                                        <div class="mb-2"><img
                                                ...
                                                class="rounded-1 shadow-sm" style="width: 40px;"></div>
                                        <div
                                            class="small text-truncate w-100 text-center flag-label">{{ $flag->name }}</div>
                                    </div>
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div class="row g-4">
                        <div class="col-12"><label
                                class="form-label small fw-bold text-secondary">НАЗВАНИЕ</label><input type="text"
                                                                                                       wire:model="name"
                                                                                                       class="form-control bg-dark border-0 text-white py-2 shadow-none border-focus border border-white border-opacity-5">
                        </div>
                        <div class="col-md-8"><label class="form-label small fw-bold text-secondary">IP
                                АДРЕС</label><input type="text" wire:model="ip"
                                                    class="form-control bg-dark border-0 text-white py-2 shadow-none border-focus border border-white border-opacity-5">
                        </div>
                        <div class="col-md-4"><label class="form-label small fw-bold text-secondary">ПОРТ</label><input
                                type="text" wire:model="port"
                                class="form-control bg-dark border-0 text-white py-2 shadow-none border-focus border border-white border-opacity-5">
                        </div>
                        <div class="col-12">
                            ...
                            <div
                                ...
                                <input :type="show ? 'text' : 'password'" wire:model="api_key"
                                       class="form-control bg-transparent border-0 text-white shadow-none py-2">
                                <button type="button" @click="show = !show"
                                        class="btn bg-transparent border-0 text-muted px-3 shadow-none"><i class="bi"
                                                                                                           :class="show ? 'bi-eye-fill' : 'bi-eye-slash-fill'"></i>
                                </button>
                            </div>
                        </div>
                    </div>
                    <div
                        ...
                        <a href="{{ route('nodes.index') }}" wire:navigate
                           class="btn btn-link text-secondary text-decoration-none fw-bold p-0"><i
                                class="bi bi-arrow-left me-2"></i>ОТМЕНА</a>
                        <button type="submit" class="btn btn-primary px-5 py-2 fw-bold rounded-3 shadow-sm">СОЗДАТЬ
                        </button>
                    </div>
                </form>
            </div>
